...
import { CalendarPlus, Users, Eye } from "lucide-react";
...
function EventRow({ event }: { event: Event }) {
  return (
    <Link
      ...
      className="flex items-center gap-3 p-4 rounded border-2 border-border bg-bg-card shadow-brutal-sm hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-brutal transition-all duration-100 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none"
    >
      {/* Cover thumbnail */}
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <div className="flex items-center gap-3 text-xs text-text-muted">
          <span className="flex items-center gap-1">
            <Users size={11} /> {event.attendees_count}
          </span>
          <span className="flex items-center gap-1">
            <Eye size={11} /> {formatPrice(event.is_free, event.price_min, event.price_max)}
          </span>
        </div>
      </div>
