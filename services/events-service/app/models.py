...
class Event(Base):
    ...
    # Host (stored locally — synced from auth-service)
    host_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    host_username: Mapped[str] = mapped_column(String(50), nullable=False)
    host_full_name: Mapped[str] = mapped_column(String(120), nullable=False)
    host_avatar_url: Mapped[str | None] = mapped_column(String(500))
    host_is_verified: Mapped[bool] = mapped_column(Boolean, default=False)
...
class EventAttendee(Base):
    ...
    status: Mapped[str] = mapped_column(Enum("going", "interested", name="attendance_status"), default="going")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
